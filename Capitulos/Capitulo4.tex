...
En cuanto a la decisión de elegir entre \texttt{Unity} o \texttt{Unreal Engine} fue basada en la comunidad que hay detrás de cada uno de ellos, y sobre todo, en lo que se quiere abarcar con este proyecto. \texttt{Unreal} suele ser utilizado por las empresas para juegos grandes y mas profesionales, mientras que \texttt{Unity} se puede aplicar, tanto a pequeñas como a grandes aplicaciones, siendo su aprendizaje mas intuitivo que con \texttt{Unreal}.

Un motor de juegos se especifica como la rutina de programación que permiten el diseño, la creación y la representación de un videojuego. En este caso, se adecua correctamente, ya que se pretende realizar un entorno 3D donde los usuarios perciban y aprendan movimientos controlando un avatar.

La funcionalidad de un motor de videojuegos consiste en, proveer al desarrollador un motor de renderizado para los gráficos 2D y 3D, un motor físico, sonidos, scripting, animaciones, un escenario gráfico, etc.

\texttt{Unity} es un motor de videojuegos desarrollado por la compañía \texttt{Unity} \texttt{Technologies} y utiliza los lenguajes de programación C, C++, C\# y JavaScript, 


\begin{figure}[h!]
	\centering
	\includegraphics[width=0.75\linewidth,height=0.17\textheight]{Imagenes/Capitulos/4/logo_unity}
	\caption{Logo Unity}
...
\texttt{Unity} ofrece diferentes tipos de versiones, como la personal de forma gratuita y la profesional, que es de pago. En la versión personal, es necesario promocionar el logotipo de \texttt{Unity} y además, carece de funcionalidades adicionales de renderizado del motor, que la profesional si contiene.

%\vspace{1cm}
Otro aspecto importante de \texttt{Unity} es el \texttt{Asset Store}. Se trata de una biblioteca del editor, que contiene una multitud de paquetes o \textit{Assets} comerciales y gratuitos, creados de forma continua por la comunidad o por \texttt{Unity Technologies}. Estos paquetes pueden contener modelados 3D, texturas, animaciones y demás de contenido que servirá de ayuda al proyecto al no poder contar con un experto diseñador. 
...
Como se observa en la imagen, el editor de \texttt{Unity} esta formado por varios paneles. Cabe destacar, que la estructuración básica de un juego en \texttt{Unity} se realiza mediante escenas.  De modo que, para poder tener una previsualización de los elementos que aparecen en una escena, se utiliza el panel uno, denominado \textit{Scene}. Este panel representa una dimensión 3D, donde se podrá rotar y desplazar en los tres ejes, para conseguir de esta manera, modificar todos los elementos de la escena en cualquier ángulo.
...
Los elementos incorporados a la escena, se archivan en el tercer panel, con el nombre \textit{Hierarchy}. Estos pueden ser  objetos 3D, cámaras, sonidos, luces, e incluso objetos de tipo GUI(Interfaz gráfica de usuario) para la implementación de interfaces. Los elementos se muestran de una forma jerárquica, al seleccionar uno, se expondrán sus características y componentes en el panel cuarto, denominado \textit{Inspector}. Por ejemplo, al seleccionar la cámara en el \textit{Hierarchy}, se muestra su componente \textit{transform}, el cual muestra la información de su posición y rotación, pudiendo ser modificada.


El quinto panel denominado \textit{Project}, agrupa en forma de directorio, todos los \textit{scripts}, paquetes, imágenes y archivos, relacionados con el proyecto.
\vspace{0.7cm}

La ejecución de pruebas del juego, se realiza por medio del \textit{Play mode}, situado en el sexto panel. Esta funcionalidad permite poner en funcionamiento el juego, pararlo y detener su ejecución, siendo esto, fundamental para comprobar y depurar un juego \citep{Unity_Manual}.
...
\begin{itemize}
	\item C\#: lenguaje estándar similar a C++ y \texttt{Java}.
 	\item \textit{UnityScript} : lenguaje basado en \texttt{JavaScript} propio de \texttt{Unity}.  
\end{itemize} 
Se optó por la decisión de utilizar C\#, ya que se acomoda mejor a la aplicación, puesto que no está orientada a web, y además, ofrece unas mejores características que \textit{UnityScript}.
...
Se utilizó \texttt{Visual Studio 2015} \citep{VSMS} para la depuración de \textit{scripts}, ya que proporciona una visualización de las variables utilizadas en tiempo de ejecución, y además, da la posibilidad de ejecutar el \textit{script} paso a paso. Otro factor clave fue, que con este entorno de programación se tenía experiencia previa de uso.
...
\texttt{MakeHuman} utiliza un humano estándar y mediante varios deslizadores o \textit{scrolls}, se podrán alterar los parámetros de la estatura, anchura, sexo, edad, etc. (véase Figura \ref{fig:makehuman_estandar})
...
En un principio, se pensó utilizar esta aplicación para el modelado de los personajes, debido a su intuitiva interfaz y la facilidad de exportación en formato fbx, el cual utiliza \texttt{Unity}. Pero tras investigar posibles alternativas, se encontró la aplicación \texttt{Fuse Character Creator}, la cual posibilitaba el desarrollo de los personajes con mayor calidad. De forma simultanea, se podría realizar la configuración de los personajes para su posterior animación.

\subsection{Fuse Character Creator}

Se trata de una aplicación desarrollada por Mixamo, la cual permite crear personajes 3D únicos mediante una intuitiva interfaz, separando el avatar en diferentes partes, como son la cabeza, el torso, las piernas y los brazos (véase Figura \ref{fig:fuse}). Posteriormente a la selección de las diferentes partes del cuerpo ya predefinidas por el sistema, se puede editar cada una de las partes del personaje, ya sean los rasgos de la cara, el tamaño de los brazos, la longitud de las piernas, etc. Al seleccionar la parte del cuerpo que se quiere modificar, simplemente será necesario pinchar y realizar un \textit{scrool} o deslizamiento hacia la izquierda o derecha para aumentar su tamaño. Por otro lado, para seleccionar el punto exacto donde se ubicarán las diferentes parte del cuerpo como por ejemplo el ombligo, los ojos, las rodillas, etc, se realizará un \textit{scrool} o desplazamiento hacía arriba o abajo.
...
\vspace{3cm}
Para la utilización de esta aplicación es necesario el uso del servidor Steam\footnote{http://store.steampowered.com/?l=spanish}, ya que se trata de una plataforma de distribución digital, comunicaciones y servicios multijugador, donde está alojado \texttt{Fuse Character Creator}.
...
Los productos y características de Mixamo son compatibles con todos los paquetes de software 3D, pudiendo realizar una exportación, eligiendo entre varias aplicaciones 3D como: \texttt{Unity3d, UDK, 3ds Max, Maya, zBrush, Houdini, Motionbuilder} entre otros.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.98\linewidth, height=0.38\textheight]{Imagenes/Capitulos/4/Mixamo}
	\caption{Avatar utilizando el servicio \textit{online} con animaciones de Mixamo}
	\label{fig:mixamo}
\end{figure}
...
Por lo tanto, para el uso de \texttt{Kinect} en un equipo o \textit{tablet} con un sistema operativo Windows \citep{KinectdeWindows}, se deberán cumplir unos requisitos mínimos \textit{hardware y software}:
